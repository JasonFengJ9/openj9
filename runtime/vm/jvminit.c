...
{
	JFRLogLevel logLevel = LOG_LEVEL_INVALID;

	Assert_VM_notNull(jfrLogLevel);
	if (0 == j9_cmdla_stricmp(jfrLogLevel, LOGLEVEL_STRING_TRACE)) {
		logLevel = LOG_LEVEL_TRACE;
	} else if (0 == j9_cmdla_stricmp(jfrLogLevel, LOGLEVEL_STRING_DEBUG)) {
		logLevel = LOG_LEVEL_DEBUG;
	} else if (0 == j9_cmdla_stricmp(jfrLogLevel, LOGLEVEL_STRING_INFO)) {
		logLevel = LOG_LEVEL_INFO;
	} else if (0 == j9_cmdla_stricmp(jfrLogLevel, LOGLEVEL_STRING_WARN)) {
		logLevel = LOG_LEVEL_WARN;
	} else if (0 == j9_cmdla_stricmp(jfrLogLevel, LOGLEVEL_STRING_ERROR)) {
...
parseJFRTags(char *jfrTags, BOOLEAN *wildcardSpecified)
{
	...
	UDATA tagMask = COMBINATION_JFRTAG_INVAID;
...
parseXlogJFR(J9JavaVM *vm)
{
	...
	/* By default, JFR log level is LOG_LEVEL_WARN. */
	for (int i = 0; i < COMBINATION_JFRTAG_COUNT; i++) {
		vm->jfrState.jfrLogTagSet[i] = LOG_LEVEL_WARN;
	}
	while (xlogindex >= 0) {
		...
		while (TRUE) {
			...
			JFRLogLevel logLevel = LOG_LEVEL_WARN;
